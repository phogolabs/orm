...
func OrderBy(clauses ...string) *Order {
	...
	for _, clause := range clauses {
		...
		// parse each element
		for _, expr := range exprs {
			if column := OrderColumnBy(expr); column != nil {
				// add the expression the collection
				order.columns = append(order.columns, column)
			}
		}
	}
...
func OrderColumnBy(expr string) *OrderColumn {
	...
	default:
		return &OrderColumn{
			err: fmt.Errorf("expression %q is not valid", expr),
		}
	}
}
